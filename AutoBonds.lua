--[[
    ...
    Focus: Auto-starts, controlled by external getgenv flag, robust cleanup.
]]
...
local CONFIG = {
    ...
    SPEED = {
        ...
        equipItemDelay = 0.2,
        equipSequenceLoopDelay = 0.7
    },
    BOND_COLLECTION_RADIUS = 75,
    MIN_PATH_SCAN_TIME = 20,
    MAX_TELEPORT_ATTEMPTS_PER_BOND = 4,
...
local isScriptActive = false -- Internal flag to track if the main process is supposed to be running
local shouldScriptRunGlobal = true -- Master control switch, tied to external toggle

-- GUI Elements (forward declaration)
local screenGui, uiContainer, bondStatusLabel
local notificationGuiStore, notificationContainer -- For notifications
...
local function initializeCharacter()
    ...
    if not hrp then warn("[VERBAL Hub] CRITICAL: HumanoidRootPart not found after wait!") end
    if not humanoid then warn("[VERBAL Hub] CRITICAL: Humanoid not found after wait!") end
    return char and hrp and humanoid
end
...
local function updateBondCountDisplay()
    if bondStatusLabel and bondStatusLabel.Parent then
        local totalText
        if not isScriptActive and #foundBondsData == 0 then
            totalText = "N/A"
        elseif isScriptActive and totalBondsToCollect == 0 and #foundBondsData == 0 then
            totalText = "Scanning..."
        elseif totalBondsToCollect == math.huge then
            totalText = "???"
        else
...
-- Scan for Bonds (during path navigation)
    ...
    for _, m in ipairs(Workspace.RuntimeItems:GetChildren()) do
        ...
        if m:IsA("Model") and m.Name == "Bond" and m.PrimaryPart then
            ...
            for _, foundData in ipairs(foundBondsData) do
                if (foundData.position - bondPos).Magnitude < 0.5 then
                    exists = true
                    break
                end
            end
...
local function attemptCollectSpecificBondModel(bondModelInstance)
    ...
    if typeof(fireclickdetector) == "function" then
        ...
        if clickDetector then
            local suc_cd, err_cd = pcall(function() fireclickdetector(clickDetector) end)
            if suc_cd then collected = true
            else warn("[VERBAL Hub] ClickDetector FAILED for", bondModelInstance.Name, ":", tostring(err_cd)) end
        end
    end
    ...
    if not collected then
        ...
        if C_ActivateObject then
            local suc_remote, err_remote = pcall(function() C_ActivateObject:FireServer(bondModelInstance) end)
            if suc_remote then collected = true
            else warn("[VERBAL Hub] C_ActivateObject FAILED for", bondModelInstance.Name, ":", tostring(err_remote)) end
        end
    end
    return collected
end
...
local function runMainBondCollectionProcess()
    if not initializeCharacter() then
        ...
        shouldScriptRunGlobal = false -- If char fails, script probably can't run
        return
    end
    isScriptActive = true
    ...
    if not eventConnections["ScanHeartbeat"] then
    ...
    for i, targetPos in ipairs(pathPoints) do
        if not shouldScriptRunGlobal or not hrp or not humanoid or humanoid.Health <= 0 then
            warn("[VERBAL Hub] Aborting path navigation due to script stop or character issue.")
            break
        end
        local dist = (hrp.Position - targetPos).Magnitude
        if dist < 1 then task.wait(); continue end
        ...
        while tween.PlaybackState == Enum.PlaybackState.Playing and (tick() - startedWaiting) < (tweenDuration + 2) do
            if not shouldScriptRunGlobal or not hrp or not humanoid or humanoid.Health <= 0 then
                tween:Cancel()
                warn("[VERBAL Hub] Tween interrupted.")
                break
            end
            task.wait()
        end
    ...
    local pathScanStartTime = tick() -- This was defined earlier, but the value is from the start of path scan.
    local timeTakenForPathScan = tick() - pathScanStartTime -- This will be very small if pathScanStartTime is redefined here.
                                                            -- It should be defined *before* the path navigation loop.
                                                            -- For now, I'll assume this logic is for post-scan delay.
    if timeTakenForPathScan < CONFIG.MIN_PATH_SCAN_TIME then -- This logic might need pathScanStartTime defined before the loop.
        local waitDuration = CONFIG.MIN_PATH_SCAN_TIME - timeTakenForPathScan
        if waitDuration > 0 then
             print("[VERBAL Hub] Path scan phase, ensuring min time, waiting for", string.format("%.1f", waitDuration), "seconds.")
    ...
    pcall(function()
        ...
        loadstring(game:HttpGet(castleScriptUrl, true))()
    end)
    task.wait(2.5); if not shouldScriptRunGlobal then print("[VERBAL Hub] Script stopped after castle TP."); isScriptActive = false; return end

    local function equipItem(itemNameQuery)
        if not shouldScriptRunGlobal or not hrp then return false end
        local itemModel = nil
        if not Workspace:FindFirstChild("RuntimeItems") then return false end
        for _, m in ipairs(Workspace.RuntimeItems:GetChildren()) do
        ...
        if itemModel then
            ...
            if equipRemote then
                local suc, err = pcall(function() equipRemote:FireServer(itemModel) end)
    ...
    for bondIdx, bondData in ipairs(foundBondsData) do
        ...
        hrp.CFrame = CFrame.new(bondData.position + Vector3.new(0, 3.5, 0))
        ...
        repeat
            ...
            task.wait(0.01) -- Tiny yield for model disappearance check
            if not bondData.model or not bondData.model.Parent then
                if not bondData.collected then
                    print("[VERBAL Hub] Bond model", bondIdx, "disappeared. Confirmed collected.")
                    bondData.collected = true
                    collectedBondsCount = math.min(collectedBondsCount + 1, totalBondsToCollect)
                    updateBondCountDisplay()
                end
                break
            end
            if bondData.collected then break end
        ...
        if not bondData.collected and shouldScriptRunGlobal then
            warn("[VERBAL Hub] Failed to confirm collection for bond", bondIdx, "after", bondData.attempts, "attempts.")
        end
    ...
    if endDecisionRemote then
        ...
        pcall(function() endDecisionRemote:FireServer(false) end)
    else warn("[VERBAL Hub] EndDecision remote not found.") end

    print("[VERBAL Hub] Main process finished.")
    isScriptActive = false
    getgenv().VerbalHubDrBondActive = false -- Allow re-activation if toggle is flipped
    
    -- If the external toggle is still true, it might loop or re-run if the script is designed that way by the loader.
    -- For this script, it means it has completed one full cycle.
    -- The external toggle monitoring will handle permanent shutdown.
end

-- === VERBAL Hub GUI Setup ===
local function setupVerbalHubGui()
    ...
    local blur = Instance.new("BlurEffect", Lighting); blur.Name = "VerbalHubBlur"; blur.Size = 0; blur.Enabled = false
    ...
    local function toggleMinimize()
        ...
        if not uiContainer or not uiContainer.Parent then return end -- Guard against destroyed UI
        local uiContainerSizeY = uiContainer.AbsoluteSize.Y > 0 and uiContainer.AbsoluteSize.Y or 250
        ...
        if compactButton and compactButton.Parent then if compactVisible then compactButton.Visible=true end; TweenService:Create(compactButton,TweenInfo.new(0.3,Enum.EasingStyle.Sine,Enum.EasingDirection.Out),{Position=compactTargetPos}):Play(); if not compactVisible then task.delay(0.3,function() if not isMinimized and compactButton.Parent then compactButton.Visible=false end end) end end
    end
...
local function setupNotificationSystem()
    ...
    if not notificationContainer:FindFirstChild("UIListLayout") then
        ...
        Instance.new("UIScale",notificationContainer).Name = "Scale"
...
local function notify(titleText, messageText, duration)
    if not notificationContainer or not notificationContainer.Parent then setupNotificationSystem() end -- Ensure it's setup
    ...
    if closeBtn and closeBtn.Parent then eventConnections["NotifClose_"..notif:GetFullName()] = closeBtn.MouseButton1Click:Connect(closeNotif) end -- Store connection
...
cleanupAndStopScript = function()
    ...
    shouldScriptRunGlobal = false -- Signal all loops to stop
    isScriptActive = false
    getgenv().VerbalHubDrBondActive = false -- For external check if main process is running

    if mainProcessCoroutine and coroutine.status(mainProcessCoroutine) ~= "dead" then
        -- Coroutines cannot be forcibly killed from outside in Lua in a clean way.
        -- The 'shouldScriptRunGlobal' flag is used to make it exit gracefully.
        -- We can try to error it if it has a debug hook, but that's advanced.
...
local function monitorExternalToggle()
    while task.wait(CONFIG.EXTERNAL_TOGGLE_CHECK_INTERVAL) do
        local deadRailsEnv = getgenv().DeadRails
        local farmEnabled = false
        if deadRailsEnv and deadRailsEnv.Farm and typeof(deadRailsEnv.Farm.Enabled) == "boolean" then
            farmEnabled = deadRailsEnv.Farm.Enabled
        end

        if not farmEnabled and shouldScriptRunGlobal then -- If toggle was turned OFF
            print("[VERBAL Hub] External toggle (DeadRails.Farm.Enabled) is now false. Stopping script.")
            cleanupAndStopScript()
            break -- Stop monitoring if script is fully shut down
        elseif farmEnabled and not shouldScriptRunGlobal and not getgenv().VerbalHubDrBondLoaded then
            ...
            -- This monitor loop itself will break after cleanup.
        end
    end
...
if not initializeCharacter() then
    ...
    cleanupAndStopScript() -- Full cleanup if char init fails
    return
end

if not eventConnections["CharacterAdded"] then
    eventConnections["CharacterAdded"] = player.CharacterAdded:Connect(function()
        task.wait(0.5)
        if not initializeCharacter() then
            warn("[VERBAL Hub] Failed to re-initialize character on CharacterAdded. May affect stability.")
            if shouldScriptRunGlobal then cleanupAndStopScript() end -- Stop if char becomes invalid
        end
    end)
end
...
pcall(setupNotificationSystem)
updateBondCountDisplay()

notify("VERBAL Hub", "Auto Bond (Externally Controlled) Initialized.", 7)
print("[VERBAL Hub] Auto Bond script ready. Waiting for external toggle or auto-starting.")

-- Start the main process if the toggle implies it should run
local initialFarmEnabled = false
local deadRailsEnv = getgenv().DeadRails
if deadRailsEnv and deadRailsEnv.Farm and typeof(deadRailsEnv.Farm.Enabled) == "boolean" then
    initialFarmEnabled = deadRailsEnv.Farm.Enabled
end
...
if initialFarmEnabled then
    if not getgenv().VerbalHubDrBondActive then
        ...
        print("[VERBAL Hub] Main process already marked active, not auto-starting new instance.")
    end
else
    print("[VERBAL Hub] External toggle DeadRails.Farm.Enabled is false or not found. Script will wait for toggle.")
    -- The script is loaded, GUI is up, but main farm won't run until toggle is true.
    -- The monitorExternalToggle will handle if it's turned on later (though the loader usually re-runs the script).
    -- For this setup, if it's initially false, the script just sits idle until the toggle is turned off (which would trigger cleanup).
    -- Or, if the loader re-executes this script when toggle becomes true, then the above 'if initialFarmEnabled' will pass.
...
-- Start monitoring the external toggle
